...
if [ "$FOUND_GIT" -ne '0' ] && [ ! -f /.opesho-puppet-stuff/librarian-puppet-installed ]; then
...
if [ "$OS" == 'debian' ] || [ "$OS" == 'ubuntu' ]; then
    if [[ ! -f /.opesho-puppet-stuff/librarian-base-packages ]]; then
        ...
        touch /.opesho-puppet-stuff/librarian-base-packages
    fi
fi

if [ "$OS" == 'ubuntu' ]; then
    if [[ ! -f /.opesho-puppet-stuff/librarian-libgemplugin-ruby ]]; then
        ...
        touch /.opesho-puppet-stuff/librarian-libgemplugin-ruby
    fi

    if [ "$CODENAME" == 'lucid' ] && [ ! -f /.opesho-puppet-stuff/librarian-rubygems-update ]; then
        ...
        touch /.opesho-puppet-stuff/librarian-rubygems-update
    fi
fi

if [[ ! -f /.opesho-puppet-stuff/librarian-puppet-installed ]]; then
    ...
    touch /.opesho-puppet-stuff/librarian-puppet-installed
else
