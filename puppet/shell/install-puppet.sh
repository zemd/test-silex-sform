...
if [[ ! -f /.opesho-puppet-stuff/update-puppet ]]; then
    if [ "$OS" == 'debian' ] || [ "$OS" == 'ubuntu' ]; then
        echo "Downloading http://apt.puppetlabs.com/puppetlabs-release-${CODENAME}.deb"
        wget --quiet --tries=5 --timeout=10 -O "/.opesho-puppet-stuff/puppetlabs-release-${CODENAME}.deb" "http://apt.puppetlabs.com/puppetlabs-release-${CODENAME}.deb"
        echo "Finished downloading http://apt.puppetlabs.com/puppetlabs-release-${CODENAME}.deb"

        dpkg -i "/.opesho-puppet-stuff/puppetlabs-release-${CODENAME}.deb" >/dev/null
        ...
        touch /.opesho-puppet-stuff/update-puppet
        echo "Created empty file /.opesho-puppet-stuff/update-puppet"
    elif [ "$OS" == 'centos' ]; then
